...
import { ArrowLeft, Edit, Trash2, Share2, MoreVertical, Lock, Globe, Calendar } from "lucide-react"
...
export function LookbookDetailsHeader({ lookbook, onEdit, onDelete }: LookbookDetailsHeaderProps) {
    ...
    <header className="relative">
      ...
      <div className="relative h-64 bg-muted">
        ...
        <div className="absolute top-4 left-4 right-4 flex items-center justify-between">
          <Button variant="ghost" size="sm" onClick={handleBack} className="bg-black/20 text-white hover:bg-black/40">
            <ArrowLeft className="w-4 h-4" />
          </Button>

          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="ghost" size="sm" className="bg-black/20 text-white hover:bg-black/40">
      ...
      <div className="p-4 space-y-4">
        ...
        <div className="space-y-2">
          <div className="flex items-start justify-between gap-3">
            <h1 className="text-2xl font-bold leading-tight">{lookbook.name}</h1>
            <Badge variant="secondary" className="text-sm">
              {lookbook.lookCount} {lookbook.lookCount === 1 ? "look" : "looks"}
            </Badge>
          </div>

          {lookbook.description && <p className="text-muted-foreground leading-relaxed">{lookbook.description}</p>}
        </div>
