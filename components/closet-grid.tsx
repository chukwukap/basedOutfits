...
export function ClosetGrid({ searchQuery, selectedFilter, sortBy, onLookClick }: ClosetGridProps) {
  const filteredAndSortedLooks = useMemo(() => {
    ...
    if (searchQuery) {
      filtered = filtered.filter(
        (look) =>
          ...
          look.brands.some((brand) => brand.toLowerCase().includes(searchQuery.toLowerCase())),
      )
    }
    ...
    if (selectedFilter !== "all") {
      if (selectedFilter === "recent") {
        ...
        filtered = filtered.filter((look) => look.category === selectedFilter || look.tags.includes(selectedFilter))
      }
    }
  ...
  if (filteredAndSortedLooks.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-12 text-center">
        ...
        <h3 className="font-semibold text-lg mb-2">{searchQuery ? "No looks found" : "Your closet is empty"}</h3>
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <p className="text-sm text-muted-foreground">
          {filteredAndSortedLooks.length} look{filteredAndSortedLooks.length !== 1 ? "s" : ""} found
        </p>
      </div>
        ...
        {filteredAndSortedLooks.map((look) => (
          ...
          >
            <div className="relative aspect-square">
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity">
                <div className="absolute bottom-2 left-2 right-2">
                  <div className="flex items-center justify-between text-white text-xs">
                    ...
                    {look.isFavorite && <Heart className="w-3 h-3 fill-current text-pink-400" />}
                  </div>
                </div>
              </div>
              ...
              <div className="absolute top-2 right-2">
                <Badge variant="secondary" className="bg-black/50 text-white border-0 text-xs backdrop-blur-sm">
                  ...
                  {new Date(look.collectedAt).toLocaleDateString("en-US", { month: "short", day: "numeric" })}
                </Badge>
              </div>
            </div>
            ...
            <div className="p-3 space-y-2">
              <div>
                <h4 className="font-medium text-sm leading-tight line-clamp-1">{look.title}</h4>
                <p className="text-xs text-muted-foreground">by {look.author}</p>
              </div>
              ...
              {look.brands.length > 0 && (
                <div className="flex flex-wrap gap-1">
                  {look.brands.slice(0, 2).map((brand) => (
                    <Badge key={brand} variant="outline" className="text-xs px-2 py-0 h-5">
                      {brand}
                    </Badge>
                  ))}
                </div>
