...
export function DiscoverCreators({ selectedTag }: DiscoverCreatorsProps) {
  ...
  const filteredCreators = selectedTag
    ? suggestedCreators.filter((creator) => creator.tags.some((tag) => tag.toLowerCase() === selectedTag.toLowerCase()))
    : suggestedCreators
    ...
    <div className="px-4 py-6 border-t bg-muted/20">
      <div className="flex items-center justify-between mb-4">
        <h3 className="font-semibold text-lg">
          {selectedTag ? `Creators posting #${selectedTag}` : "Discover Creators"}
        </h3>
      ...
      <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide">
        {filteredCreators.map((creator) => (
          <Card key={creator.id} className="flex-shrink-0 w-40 p-4 text-center">
            ...
            <h4 className="font-medium text-sm mb-1 truncate">{creator.name}</h4>
            <p className="text-xs text-muted-foreground mb-1">{creator.username}</p>
            <p className="text-xs text-muted-foreground mb-3">{creator.followers} followers</p>
