  ...
  useEffect(() => {
    ...
    const syncUser = async () => {
      try {
        ...
        const fid: string | undefined = ((c?.user?.fid ?? c?.client?.fid) as number | string | undefined)?.toString();
        const username: string | undefined = (c?.user?.username ?? c?.client?.username) as string | undefined;
        const name: string | undefined = (c?.user?.displayName ?? c?.client?.displayName) as string | undefined;
        const avatarUrl: string | undefined = (c?.user?.pfpUrl ?? c?.client?.pfpUrl) as string | undefined;
