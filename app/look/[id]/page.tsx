...
import { LookDetailView } from "@/components/look-detail-view"
...
const mockLookData = {
  "1": {
    ...
    images: ["/fashionable-summer-outfit.png", "/summer-fashion-outfit.png", "/stylish-person-streetwear.png"],
    author: {
      name: "Sarah Chen",
    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-muted-foreground mb-4">This look might have been removed or doesn't exist.</p>
    ...
    <div className="min-h-screen bg-background pb-20">
      ...
      <header className="sticky top-0 z-20 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 border-b">
        <div className="flex items-center justify-between p-4">
          ...
          <h1 className="text-lg font-semibold truncate flex-1 mx-4">{look.title}</h1>
      ...
      <div className="px-4 py-6">
        <h3 className="text-lg font-semibold mb-4">Other Looks by {look.author.name}</h3>
        <div className="flex gap-3 overflow-x-auto pb-2">
          {/* Mock other looks by same author */}
          {[1, 2, 3, 4].map((i) => (
            <div key={i} className="flex-shrink-0 w-32 cursor-pointer" onClick={() => router.push(`/look/${i + 10}`)}>
              ...
              <p className="text-xs text-muted-foreground truncate">Look Title {i}</p>
            </div>
          ))}
        </div>
      </div>
